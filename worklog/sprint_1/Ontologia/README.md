...
- **`Artwork`**: opera d'arte generica, con codice identificativo, titolo e descrizione. Si suddivide in **`Painting`** (dipinto), l'unico sottotipo popolato in questo progetto, che porta le relazioni verso l'istituto e verso il riferimento temporale.
- **`CulturalInstituteOrSite`**: l'istituto o luogo che ospita un dipinto.
- **`Place`**: il luogo geografico (comune) in cui si trova un istituto, identificato dal codice ISTAT.
- **`TimeReference`**: il riferimento temporale di un dipinto, specializzato in tre sottoclassi disgiunte: **`ExactDate`** (una data AAAA-MM-DD), **`YearRange`** (un intervallo tra due anni), **`CenturyReference`** (un secolo, con numero e parte opzionale: inizio, metà, fine, prima metà, seconda metà).

Le relazioni principali:
- un dipinto **è ospitato in** un istituto (`isHostedIn`, e viceversa **ospita**, `hosts`);
- un istituto **si trova in** un luogo (`isLocatedIn`, e viceversa **ha istituto**, `hasCulturalInstitute`);
- un dipinto **ha un riferimento temporale** (`hasTimeReference`). A differenza delle prime due relazioni, questa può collegare più dipinti allo stesso riferimento temporale: se due opere hanno la stessa data, condividono lo stesso valore invece di averne uno ciascuna. È lo stesso comportamento previsto nello schema E-R, dove questa relazione ha cardinalità (0,N).
...
Il disegno [`Diagrammi/Graffoo_Onto-PM.drawio.png`](./Diagrammi/Graffoo_Onto-PM.drawio.png) rappresenta graficamente questo modello secondo la notazione Graffoo. Lo schema [`Diagrammi/DiagrammaER_Onto-PM.drawio.png`](./Diagrammi/DiagrammaER_Onto-PM.drawio.png) ne dà una lettura più semplice, orientata alla progettazione dei dati piuttosto che alla formalizzazione OWL. I due diagrammi non sono due modelli diversi, raccontano  le stesse classi e le stesse relazioni, solo con un livello di dettaglio diverso: 

### 1. Diagramma Entity-Relation
![E-R](Diagrammi/DiagrammaER_Onto-PM.drawio.png)

Il **diagramma E-R** è una versione semplificata, pensata per progettare i dati e farsi capire subito da chi legge.

### 2. Graffoo
![Graffoo](Diagrammi/Graffoo_Onto-PM.drawio.png)

Il **Graffoo** invece è la versione completa e precisa, con tutti i dettagli tecnici (tipi di dato, regole, vincoli) che servono per trasformare il modello in un'ontologia vera e propria, leggibile da un computer. 

---

## 2. Competency question

Il file CQ_Onto-PM.txt elenca le domande a cui l'ontologia deve saper rispondere. Le abbiamo riviste più volte mentre il modello prendeva forma.
...
Questa cartella fa parte dell'epic **Semantica e ontologie** (PM-3). La pagina che espone il disegno Graffoo in forma divulgativa al pubblico del sito è prodotta separatamente nella **US1.3** (Sprint 2). Le competency question elencate sono la base delle query SPARQL di esempio documentate in **US1.4**.
